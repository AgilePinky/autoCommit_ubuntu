...
public class CheckRRS_2 {
    private WebDriver driver;

    public CheckRRS_2(WebDriver driver) {
        this.driver = driver;
    }
...
//            String[] namespaceValues = {"devtcn", "cat", "uz", "kg"};
            ...
            for(int i = 0; i < namespaceArray.length && i < namespaceValues.length; i++) {
                if (namespaceArray[i]) {
                    ...
                    // 3. Выбираем react-renderer-service
                    wait.until(ExpectedConditions.visibilityOfElementLocated(
                            By.xpath("//a[@class='variable-option pointer']//span[text()='react-renderer-service']"))
                    ).click();
                    System.out.println("Check-box react-renderer-service selected");
                    actions.sendKeys(Keys.ENTER).perform();
                    Thread.sleep(250);

                    // 4. Проверяем текст
                    WebElement textInputElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                            By.xpath("//div[contains(text(), 'react-renderer-service')]")));
                    ...
                    if (actualTextInput != null && actualTextInput.contains("react-renderer-service")) {
                        try {
                            // 5. Делаем скриншот
                            String screenshotName = "resources/Screenshot_RRS_" + namespaceValues[i] + ".png";
