...
public class CheckFCS_2 {
    private WebDriver driver;

    public CheckFCS_2(WebDriver driver) {
        this.driver = driver;
    }
...
//            String[] namespaceValues = {"devtcn", "cat", "uz", "kg"};
            ...
            for(int i = 0; i < namespaceArray.length && i < namespaceValues.length; i++) {
                if (namespaceArray[i]) {
                    ...
                    // 3. Выбираем front-content
                    wait.until(ExpectedConditions.visibilityOfElementLocated(
                            By.xpath("//a[@class='variable-option pointer']//span[text()='front-content']"))
                    ).click();
                    System.out.println("Check-box front-content selected");
                    actions.sendKeys(Keys.ENTER).perform();
                    Thread.sleep(250);

                    // 4. Проверяем текст
                    WebElement textInputElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                            By.xpath("//div[contains(text(), 'front-content')]")));
                    ...
                    if (actualTextInput != null && actualTextInput.contains("front-content")) {
                        try {
                            // 5. Делаем скриншот
                            String screenshotName = "resources/Screenshot_FCS_" + namespaceValues[i] + ".png";
